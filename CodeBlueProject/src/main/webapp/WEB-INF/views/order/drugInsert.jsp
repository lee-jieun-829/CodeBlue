...
<script>
    ...
    function insertModal() {
        ...
        if (emptyQtyItem) {
            const dName = emptyQtyItem.drugName;
            if(typeof Swal !== 'undefined') Swal.fire('확인 필요', `[\${dName}]의 수량을 입력해주세요.`, 'warning');
            else alert(`[\${dName}]의 수량을 입력해주세요.`);
            return;
        }
        ...
        cartList.forEach((item, index) => {
            ...
            html += `
                <tr>
                    ...
                    <td>
                        <span>\${qty}\${item.drugUnit}</span>
                    </td>
                    <td class="text-right">
                        \${(item.drugCost*qty).toLocaleString()}원
                    </td>
                </tr>
            `;
        });
    ...
    function submitBulkOrder() {   	
    	...
    	axios.post('/order/orderInsert', orderData, {
  	    ...
  	    .then(function (response) {
			sendNewNotification(26030909, '약품 발주 신청', '새로운 약품 발주 신청이 접수되었습니다.', '004', 'http://localhost:5173/admin/orders');	        	
            // 성공 시 처리
  	    	Swal.fire({
                title: '발주 신청 완료',
    ...
    function processAfterSubmit() {
        cartList = []; // 장바구니 비우기
        renderCart();  // 메인 화면 장바구니 UI 갱신 (drug.jsp에 있는 함수)
        closeModal('insertModal'); // 모달 닫기 (drug.jsp에 있는 함수)
    }
</script>