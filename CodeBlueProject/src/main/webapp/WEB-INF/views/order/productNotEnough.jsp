    ...
    <div class="modal modal-md">
        <div class="modal-header" style="border-bottom: 1px solid var(--color-danger-light);">
            <div style="display: flex; align-items: center; gap: var(--spacing-sm);">
                ...
                <h3 class="modal-title" style="color: var(--color-danger);">재고 부족 물품 목록</h3>
            </div>
...
<script>
    ...
    function checkLowStock() {
    	axios.get('/order/notEnoughProduct')
    	 .then(function (response) {
    		 fetchedLowStockList = response.data;
    		 console.log("listContainer",fetchedLowStockList);
    		 if (!fetchedLowStockList || fetchedLowStockList.length === 0) {
    			 sweetAlert("success", "재고 부족 물품이 없습니다.", "확인");
    		 }
                	 ...
                	 <table class="table">	                     
	                     <tbody>
	                         <tr>
	                             ...
	                             <div style="font-size: var(--font-sm);"> 현재: \${item.productAmount} 개  / 안전재고: \${item.productSaftyStoke} 개</div>
	                             </td>
	                             <td style="font-size: var(--font-sm);"> </td>	                             
    ...
    function addAllLowStockToCart() {
	    ...
	    if (skippedNames.length > 0) {      
	        msgHtml += `<span style="font-size: var(--font-sm); color: var(--color-text-secondary); margin-bottom: var(--spacing-xs);">
	        				이미 담겨있는 <br/>[ \${skippedNames[0]} 외 \${skippedNames.length}건 ]<br/> 물품은 제외 하였습니다.
	        			</span>`;
	    }
