						...
						<div class="card  flex-1 overflow-y-auto min-h-0 my-2 pr-2 scrollbar">
							 <div class="h-[15%] card-header  flex-none" style="font-size: var(--font-base); font-weight: var(--font-semibold); ">
								 ...
								 <div class="btn-group-right">
								    <button onclick="checkLowStock()" class="btn btn-destructive btn-sm">⚠ 부족수량 물품 확인</button>
								    <input type="text" class="input input-search input-sm" placeholder="검색어를 입력하세요" id="searchKeyword">
								    <button class="btn btn-primary btn-sm">확인</button>
								</div> 
							 </div>
							 <div class="h-[78%] card-body  flex-1 overflow-y-auto">
							 	<!-- 물품리스트 출력 영역 -->
							 	<div id="productList" class="table-wrapper">								    
								</div>							
				    ...
				    <div class="card h-full flex flex-col min-h-0 my-2">				        
				        ...
				        <div class="card-body  flex-1 overflow-y-auto scrollbar">
				            ...
				            <table class="table table-center table-sm">
				            	<!-- 선택한 물품 출력 영역 -->				            	
				            	<tbody id="selectProduct"class="w-full">
				            		<tr>
						                <td colspan="6" style="padding: 2rem;">
						                    <div class="empty-state empty-state-sm" style="border:none;">
						                    	<svg class="empty-state-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
						                         ...
						                         <div class="empty-state-description">담긴 물품이 없습니다.</div>
						                    </div>
						                </td>
						            </tr>				            						            		
				            	</tbody>				            	
...
<jsp:include page="/WEB-INF/views/order/productInsert.jsp" />
<jsp:include page="/WEB-INF/views/order/productNotEnough.jsp" />
<script>
	    ...
	    axios.get('/order/productList',{
	        ...
	        .then(function (response) {	        	
	            ...
	            if(globalProductList && globalProductList.length > 0) {
	            	globalProductList.forEach(function(product, index) {
                    // 수량 상태 로직 
                    let amountHtml = '';
                    let amount = '';
                    if(product.productAmount > product.productSaftyStoke) {
                         // 정상
                         amountHtml = `<span style="color: var(--color-primary); font-weight: var(--font-semibold);">\${product.productAmount}</span>`;
                    } else {
                         // 부족
                         amountHtml = `
                        	 <span class="badge badge-danger">부족</span>
                         	 <span style="color: var(--color-destructive); font-weight: var(--font-semibold);">\${product.productAmount}</span> 
                                       `;
                    }
					if(product.productType =="001") {
						productType ="생필품";                
					} else if(product.productType =="002"){
						productType ="의료용품";                     
					}else{
						productType ="사무용품";         
					}
                    ...
                    html += `
                        <tr>							
                            <td class="text-center">\${product.productNo}</td>
							<td class="text-center">\${productType}</td>
                            <td>\${product.productName}</td>
                            <td >\${product.productCompany}</td>
                            <td class="text-right">\${amountHtml} 개</td>
                            <td class="text-right">\${product.productSaftyStoke} 개</td>
                            <td class="text-right">\${product.productCost.toLocaleString()}원</td>                             
                            <td class="text-center"><button onclick="selectProduct(\${index})" class="btn btn-primary btn-sm">발주</button></td>
                        </tr>
                    `;
                });
	          } else {
	          ...
	          document.getElementById('productList').innerHTML = html;
			   renderPagination(currPage + 1, totalPages, 'pageNation', 'loadProductList');
			   
	        })
	        .catch(function (error) {	            
	        	sweetAlert("warning", "서버오류, 물품리스트를 가져올 수 없습니다.", "확인");
	        });
	}
	...
	function selectProduct(index){
		const product = globalProductList[index];
		//장바구니 담겨있는지 확인용 플래그
		let isExist = false;
		
	    for (let i = 0; i < cartList.length; i++) {
	        if (cartList[i].productNo === product.productNo) {
	            isExist = true; 
	            break;          
	        }
	    }
	    ...
	    if (isExist) {
	       sweetAlert("warning", "이미 장바구니에 담긴 물품입니다.", "확인");
	    }
	...
	//==========================================================================
	function renderCart() {
		let selectProductBody = document.getElementById('selectProduct');			
		let html ='';
			...
			cartList.forEach((product, index) => {
		        ...
		        html += `
		            <tr class="items-center border-b" style="font-weight: var(--font-semibold);">
		                ...
		                <td style="text-align:left">\${product.productCompany}</td>
		                <td style="text-align:right">\${product.productCost.toLocaleString()}원</td> 
		                <td style="width: 20%;">
			                <div style="display: flex; align-items: center; gap: 5px; justify-content: center;">
				                <input type="number" class="input" value="\${product.orderQty}"
		...
		selectProductBody.innerHTML = html;
