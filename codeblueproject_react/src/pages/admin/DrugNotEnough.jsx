...
const DrugNotEnough = ({ isOpen, onClose, cartList, setCartList }) => {
    ...
    const fetchLowStockList = async () => {
        try {            
            const res = await axios.get('http://localhost:8060/api/order/notEnoughDrug', {
    ...
    const addAllItems = () => {
        ...
        if (skippedNames.length > 0) {
             msgHtml += `<div style="font-size: 0.875rem; color: #6b7280; margin-top: 8px;">
                            이미 담겨있는 <br/>[ ${skippedNames[0]} 외 ${skippedNames.length - 1}건 ]<br/> 약품은 제외 하였습니다.
                        </div>`;
        }
            ...
            <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl mx-4 overflow-hidden flex flex-col max-h-[80vh]">
                ...
                <div className="flex justify-between items-center p-4 border-b border-red-200 bg-red-50">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-lg font-bold text-red-600">재고 부족 약품 목록</h3>
                    </div>
                ...
                <div className="p-4 overflow-y-auto flex-1">
                    {lowStockList.map((item) => (
                        <div key={item.drugNo} className="border-b border-gray-100 py-2 last:border-0">
                            <div className="flex justify-between items-center">
                                {/* 약품 정보 */}
                                <div className="flex flex-col">
                                    <span className="font-semibold text-sm">{item.drugName}</span>
                                    <span className="text-xs text-gray-400">{item.drugCompany}</span>
                                </div>
                                
                                {/* 재고 정보 */}
                                <div className="text-sm text-gray-600">
                                    현재: <span className="text-red-600 font-bold">{item.drugAmount}</span> / 
                                    안전재고: {item.drugSaftyStoke}
                                </div>
