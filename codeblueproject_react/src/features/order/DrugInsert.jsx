import React, { useMemo } from 'react';
import axios from 'axios';
import Swal from 'sweetalert2';

// 백엔드 서버.. 프로토콜 살리기 프로젝트 시작합니다
const SPRING_URL = `${window.location.protocol}//${window.location.hostname}:8060`;

const DrugInsert = ({ isOpen, onClose, cartList, clearCart }) => {
  
  // 1. 총 합계금액 계산
  const grandTotal = useMemo(() => {
    if (!cartList || cartList.length === 0) return 0;
    return cartList.reduce((acc, item) => {
      const qty = parseInt(item.orderQty) || 0;
      return acc + (item.drugCost * qty);
    }, 0);
  }, [cartList]);

  // 2. 최종 신청 완료 핸들러
  const handleSubmit = async () => {
    // 유효성 검사
    const emptyQtyItem = cartList.find(item => !item.orderQty || item.orderQty <= 0);
    if (emptyQtyItem) {
      Swal.fire({
          title: '확인 필요',
          text: `[${emptyQtyItem.drugName}]의 수량을 입력해주세요.`,
          icon: 'warning',
          confirmButtonColor: '#3085d6',
          confirmButtonText: '확인'
      });
      return;
    }

    // 서버 전송 데이터
    const orderData = {
      orderTotalamt: grandTotal,
      orderType: '001',
      orderStatus: '001',
      orderDetails: cartList.map(item => ({
        orderItemNo: item.drugNo,
        orderDetailCount: parseInt(item.orderQty),
        orderItemType: '001'
      }))
    };

    try {
      await axios.post(`${SPRING_URL}/api/order/orderInsert`, orderData, {
        headers: { 'Content-Type': 'application/json' },
        withCredentials: true
      });

      Swal.fire({
        title: '발주 신청 완료',
        text: '발주 신청이 성공적으로 접수되었습니다.',
        icon: 'success',
        confirmButtonColor: '#2563eb', // blue-600
        confirmButtonText: '확인'
      }).then((result) => {
        if (result.isConfirmed) {
          clearCart();
          onClose();
        }
      });

    } catch (error) {
      console.error(error);
      const msg = (error.response && error.response.status === 404) 
        ? "발주 신청 처리에 실패했습니다. (결과값 없음)"
        : "서버오류, 발주 신청을 실패하였습니다";
        
      Swal.fire({
          icon: "warning",
          title: "오류",
          text: msg,
          confirmButtonColor: '#d33'
      });
    }
  };

  if (!isOpen) return null;

  return (
    // 배경 (Backdrop) - DrugNotEnough와 동일한 구조
    <div 
        className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50"
        onClick={(e) => { if(e.target === e.currentTarget) onClose(); }}
    >
        {/* 모달 창 */}
        <div className="bg-white rounded-lg shadow-lg w-full max-w-3xl mx-4 overflow-hidden flex flex-col max-h-[85vh]">
            
            {/* 헤더 (Blue 테마 적용) */}
            <div className="flex justify-between items-center p-4 border-b border-blue-200 bg-blue-50">
                <div className="flex items-center gap-2">
                    <span className="text-2xl">📝</span>
                    <h3 className="text-lg font-bold text-blue-600">통합 발주 신청</h3>
                </div>
                <button onClick={onClose} className="text-gray-500 hover:text-gray-700 text-2xl font-bold">&times;</button>
            </div>
            
            {/* 바디 (스크롤 영역) */}
            <div className="p-6 overflow-y-auto flex-1">
                <div className="mb-4 text-sm text-gray-500 p-3  border-gray-100">
                    선택하신 약품의 발주 내역을 최종 확인해주세요.
                </div>

                <div className="border rounded-lg overflow-hidden border-gray-200">
                    <table className="w-full text-sm text-left">
                        <thead className="text-gray-700 bg-gray-100 border-b border-gray-200">
                            <tr>
                                <th className="px-4 py-3 w-[50px] text-center">No</th>
                                <th className="px-4 py-3">약품명 / 제조사</th>
                                <th className="px-4 py-3 text-right">단가</th>
                                <th className="px-4 py-3 text-center w-[100px]">신청수량</th>
                                <th className="px-4 py-3 text-right">주문액</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            {cartList.map((item, index) => {
                                const qty = parseInt(item.orderQty) || 0;
                                const itemTotal = item.drugCost * qty;
                                return (
                                    <tr key={item.drugNo || index} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-center text-gray-500">{index + 1}</td>
                                        <td className="px-4 py-3">
                                            <div className="font-medium text-gray-900">{item.drugName}</div>
                                            <div className="text-xs text-gray-400">{item.drugCompany}</div>
                                        </td>
                                        <td className="px-4 py-3 text-right text-gray-600">
                                            {item.drugCost.toLocaleString()}원
                                        </td>
                                        <td className="px-4 py-3 text-center">
                                            <span className="px-4 py-3 text-right font-medium">
                                                {qty}{item.drugUnit}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-right font-medium">
                                            {itemTotal.toLocaleString()}원
                                        </td>
                                    </tr>
                                );
                            })}
                        </tbody>
                    </table>
                </div>

                {/* 총계 표시 박스 */}
                <div className="mt-6 bg-slate-50 p-4 rounded-lg border border-gray-200 flex justify-end items-center gap-4 shadow-sm">
                    <span className="text-lg font-bold text-slate-500">총 합계금액</span>
                    <span className="text-2xl font-black text-blue-600">
                        {grandTotal.toLocaleString()}원
                    </span>
                </div>
            </div>
            
            {/* 푸터 */}
            <div className="p-4 border-t border-gray-200 flex justify-end gap-2 bg-gray-50">
                <button 
                    onClick={onClose}
                    className="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 font-bold text-sm"
                >
                    취소
                </button>
                <button 
                    onClick={handleSubmit}
                    className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 font-bold text-sm shadow-md"
                >
                    최종 신청 완료
                </button>
            </div>

        </div>
    </div>
  );
};

export default DrugInsert;